...
if (interaction.commandName === 'giveaway') {
   ...
   const giveawayMessage = await targetChannel.send({
    embeds: [embed],
    components: [giveawayRow, rerollRow] // ADD REROLL BUTTON
});
...
if (interaction.isButton()) {
    ...
    if (interaction.customId === 'giveaway_join') {
        ...
        await interaction.message.edit({
            embeds: [embed],
            components: [giveawayRow, rerollRow]
        });
    ...
    // --- GIVEAWAY REROLL (ONLY AFTER GIVEAWAY ENDS) ---
    if (interaction.customId === 'giveaway_reroll') {
        ...
        if (
            ...
            return interaction.reply({
                content: "Only staff or the server owner can re-roll the giveaway.",
                ephemeral: true
            });
        }

        if (client.giveaway.active === true) {
            return interaction.reply({
                content: "You can only re-roll **after** the giveaway has ended.",
                ephemeral: true
            });
        }

        if (!client.giveaway.entries || Object.keys(client.giveaway.entries).length === 0) {
            return interaction.reply({
                content: "No entries exist to re-roll.",
                ephemeral: true
            });
        }
        ...
        for (const [uid, amount] of Object.entries(client.giveaway.entries)) {
            if (amount > highest) {
                highest = amount;
                winner = uid;
            }
        ...
        return interaction.reply({
            content: `🔄 **Re‑Roll Result:** <@${winner}> has the most entries (${highest}) and is the new winner!`,
            ephemeral: false
        });
    }
    ...
    if (interaction.customId === 'called') {
        if (data.called) {
            return interaction.reply({
...
client.on('messageCreate', async message => {
    ...
    if (cmd === 'balance' || cmd === 'currency' || cmd === 'wallet') {
        ...
        if (!target && args[0] && /^\d{17,19}$/.test(args[0])) {
        ...
        if (target && message.guild?.members.cache.get(message.author.id)?.roles.cache.has(STAFF_ROLE)) {
            const bal = getPoints(target.id);
            return message.reply(`Balance for <@${target.id}>: **${bal} points**`);
        }

        const bal = getPoints(message.author.id);
        return message.reply(`Your balance: **${bal} points**`);
    }
...
// Only deduct points AFTER confirming giveaway is active for entry items
if (itemName === "entry1" || itemName === "entry2" || itemName === "instantwin") {
    if (!client.giveaway.active) {
        return message.reply("You can only buy this item during an active giveaway.");
    }
}
